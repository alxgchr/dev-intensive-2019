...
enum class TimeUnits {
    SECOND {
        override fun plural(num: Int): String {
            return "$num ${if (num%10 == 1) "секунду" else if (num%10 in 2..4) "секунды" else "секунд"}"
        }
    }, MINUTE {
        override fun plural(num: Int): String {
            return "$num ${if (num%10 == 1) "минуту" else if (num%10 in 2..4) "минуты" else "минут"}"
        }
    }, HOUR {
        override fun plural(num: Int): String {
            return "$num ${if (num%10 == 1) "час" else if (num%10 in 2..4) "часа" else "часов"}"
        }
    }, DAY {
        override fun plural(num: Int): String {
            return "$num ${if (num%10 == 1) "день" else if (num%10 in 2..4) "дня" else "дней"}"
        }
    };
...
fun Date.humanizeDiff(date: Date = Date()): String {
    ...
    val str = when {
        ...
        interval / MINUTE <=75 -> if(isFuture) {"через час"} else{"час назад"}
       // interval / HOUR <=22 -> if(isFuture) {"через ${TimeUnits.MINUTE.plural((interval/ HOUR).toInt())}"} else{"${TimeUnits.HOUR.plural((interval/ HOUR).toInt())} назад"}
        ...
        interval / HOUR <=26 -> if(isFuture) {"через день"} else{"день назад"}
